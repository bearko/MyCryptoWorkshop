...
export const STAT_INFO: Partial<Record<NumStat | 'spawnInterval', Info>> = {
  ...
  alternative: { label: t('代わりの品のご提案', 'Suggest something else'), format: (v) => (v > 0 ? t('あり', 'On') : t('なし', 'Off')) },
  shopCapacity: { label: t('店に入れる客', 'Shop capacity'), format: (v) => t(`${v}人`, `${v}`) },
  scareChance: { label: t('エネミーや泥で帰る確率', 'Chance of leaving over enemies or mud'), format: (v) => `${Math.round(v * 100)}%` },
