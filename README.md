...
| 赤く光るヒーローをクリック | 泥棒を捕まえて懸賞金を得る。盗まれた品も戻る |
| 壺にとりついたエネミーをクリック | 追い払う（いる間はクラフト速度が半分） |
...
開発サーバーでは `G` キーで +10,000 GUM、`E` キーでその日の営業を終了できます（本番ビルドでは無効）。

### 素材の取り込み

`scripts/sync-assets.mjs` が素材DBから使う分だけを選び、次の2つを生成します（どちらも Git 管理外）。

- `public/mch/**` — 使用する画像・音声のコピー（素材DBと同じパス構成）
- `src/generated/catalog.json` — ゲーム用に整形したカタログ（シリーズ、客、泥棒、エネミー、スタッフ、アイコン、音声）

使うシリーズや泥棒役のヒーロー、エネミーはスクリプト冒頭の定数で変更できます。
`MCH_MAX_CUSTOMERS_PER_TIER=16` のように指定すると、客ヒーローの数をレアリティごとに絞れます（ファイル数に上限がある配布先向け）。
